...
public interface PageRepository extends CrudRepository<Page,Long> {
    @Query(value ="SELECT CASE WHEN count(*) >0 THEN 'true' ELSE 'false' END as `boolean` FROM skillbox.pages WHERE path = :path and sites_id = :sitesID",nativeQuery = true)
    ...
    @Query(value ="SELECT Count(*) FROM skillbox.pages",nativeQuery = true)
